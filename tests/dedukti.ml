module Command = struct
  ...
  let path = function
    | Dkcheck  -> "./dedukti.native", fun args -> "check" :: args
    | Dkmeta   -> "./dkmeta.native", Fun.id
    | Dkpretty -> "./dedukti.native", fun args -> "beautify" :: args
end
...
module Pretty = struct
  ...
  let run ?(dep = []) ~filename arguments =
    ...
    let arguments = "beautify" :: (List.map mk_argument arguments |> List.concat) in
    let title =
