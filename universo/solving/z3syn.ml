...
let mk_enum ctx i = Z.Expr.mk_const_s ctx ("Enum" ^ string_of_int i) (sort ctx)
...
let mk_bounds ctx var i =
  ...
  let or_eqs =
    List.map
      (fun u -> Z.Boolean.mk_eq ctx (mk_var ctx var) (mk_univ ctx u))
      univs
  in
...
let solution_of_var ctx i model var =
  ...
  | Some e -> (
      try
        ...
        failwith
          "Variable was not found in the model, this error should be reported"
      with Found u -> u)
